...
def cadastrar_pet(pets, proximo_id):
    especie = input("Qual espécie do pet (ex: gato, cachorro, etc.): ")
...
def buscar_pet(pets):
    ...
    if encontrados:
        for id_pet, dados in encontrados:
            status = "Adotado" if dados['adotado'] else "Disponível"
            print(f"\nID: {id_pet} | Espécie: {dados['especie']} | Nome: {dados['nome']} | Idade: {dados['idade']} | Raça: {dados['raca']} | Status: {status}")
    else:
...
def remover_pet(pets):
    ...
    if id_pet in pets:
        ...
        especie = pets[id_pet]['especie']
        del pets[id_pet]
        print(f"O '{especie}' de '{nome}' foi removido com sucesso.")
    else:
        print("ID inválido.")
...
def menu():
    ...
    print("4. Atualizar informações do pet")
    print("5. Marcar como adotado")
    print("6. Remover pet do cadastro")
    print("0. Sair")
